...
define("SIMPLE_CAPTCHA_PATH", dirname(__FILE__));
class SimpleCaptcha {
    ...
    public function __construct($config = array()) {
      $this->wordsFile = SIMPLE_CAPTCHA_PATH . "/{$this->wordsFile}";
      $this->resourcesPath = SIMPLE_CAPTCHA_PATH . "/{$this->resourcesPath}";
    }
    ...
    public function CreateImage($boolDrawIamge = true) {
        $ini = microtime(true);
        ...
        ob_start();
        $this->WriteImage($boolDrawIamge);
        $this->Cleanup();
        $strContentImage = ob_get_contents();
        ob_end_clean();

        /** Output */
        if($boolDrawIamge){
          print $strContentImage;
        }

        return $strContentImage;
    }
     ...
     * File generation
     */
    protected function WriteImage($boolDrawIamge) {
    ...
    public function GetHeaders() {
        $arrReturn = array();
        if ($this->imageFormat == 'png' && function_exists('imagepng')) {
          $arrReturn["Content-type"] = "image/png";
        } else {
          $arrReturn["Content-type"] = "image/jpeg";
        }
        return $arrReturn;
    }

    private function setSession($strCaptcha) {
        $sessionName = $this->session_var;

        Session::put($sessionName, $strCaptcha);

     /* $objSessions = new \Zend\Session\Container("simplecaptcha");
      $objSessions->{$this->session_var} = $strCaptcha;*/
    }
    ...
    public function getSession() {
        ...
        $strCaptcha = Session::get($sessionName);

        return $strCaptcha;

      /*$objSessions = new \Zend\Session\Container("simplecaptcha");
      if ($objSessions) {
          $strCaptcha = $objSessions->{$this->session_var};
      }
      return $strCaptcha;*/
    }
