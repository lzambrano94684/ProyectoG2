...
class VentasController extends BaseController
{
    ...
    public function cargadas(Request $request)
    {
        ...
        //dump($arrayProductoF->get('226671'), $IdFranquiciaMA);
        ...
        $request = $this->UrlToData($request);
        $titleMsg = "Carga de Ventas Sell In";
        ...
        //dump($arrayCliente->sort(), PX_SIS_Entidad::select("CodigoCliente")->where(DB::raw("RTRIM(CodigoCliente)"), "6937")->get());
        $mes = date("m");
        $anio = date("Y");
        ...
        $totalSales = collect(collect(DB::select("select SUM(TOTAL_USD) from [dbo].[Fun_Sales]($anio, $mes, 1) as Total"))
            ->first())->first();
        $codNi = $this->codNicaragua;
        $thisController = $this;

        $subTitle = "A continuación un detalle previo a cargar las ventas";
        ...
        return view("Sistema.Pointex.Modulo.BI.Ventas.Sell_In.index", get_defined_vars());
    }
    ...
    public function saveVentas(Request $request)
    {
        ...
        $tipoZPBE = trim($request->input("Tpost"));
        ...
        $kNPedido = $request->input("cmbNPedido");
        ...
        $kTipoVenta = $request->input("cmbTipoVenta");
        $mes = $txtFecha[1];
        $anio = $txtFecha[0];
        ...
        if (strpos($tipoZPBE, "ZPBE") !== false) {
            $deleteFac = PX_BI_FacturaEncabezadoB::where(DB::raw("MONTH(Fecha)"), (int)$mes)
                ...
                ->delete();
            //echo "PX_BI_FacturaEncabezadoBdelete<br>";
        } else {

            $deleteFac = PX_BI_FacturaEncabezado::where(DB::raw("MONTH(Fecha)"), (int)$mes)
                ->where(DB::raw("YEAR(Fecha)"), $anio)
                ->delete();
            $deleteVentas = PX_BI_Ventas::where(DB::raw("MONTH(Fecha)"), (int)$mes)
                ->where(DB::raw("YEAR(Fecha)"), $anio)
                ->delete();
            //echo "PX_BI_FacturaEncabezadodelete<br>";
        }
        $arrayEntrs = [];
        ...
        foreach ($dataMasiva as $kdm => $value) {
            $IdDistribuidor = PX_SIS_ENTIDAD::from("PX_SIS_ENTIDAD as e")
                ->select("e.Id")
                ->where("e.CodigoCliente",strval($value->get($kDistribuidor)))
                ->first()
                ->Id;
            ...
            $NPedido = trim($value->get($kNPedido));
            $FechaFactura = $value->get($kFecha);

            $arrayInsertFacturaEncabezado = compact("IdDistribuidor",
                "IdPais", "NPedido", "Factura", "Referencia", "Fecha", "FechaFactura", "UsuarioCreacion");
            $IdFacturaEncabezado = $this->returnIdFactura($arrayInsertFacturaEncabezado, $tipoZPBE, $Fecha);
            $codProducto = $value->get($kProducto);
            $canalVenta = trim($value->get("ClPed"));
            if ($canalVenta == "INST" || $canalVenta == "Inst" || $canalVenta == "inst") {
                $arrayEntrs[] = 1;
                ...
                $consultaProducto = $catProducto->get($codProducto)
                    ->where("TipoPresentacion", "Venta")->last();
                if ($consultaProducto) {
                    $IdProducto = $consultaProducto->Id;
                } else {
                    $consultaProducto = $catProducto->get($codProducto)
            ...
            $TipoVenta = $value->get($kTipoVenta);
            $arrayInsertDetalleFactura = compact("IdFacturaEncabezado", "IdProducto",
                "Cantidad", "FechaVencimiento","Lote", "Precio", "Venta100", "Incoterm", "Venta97", "UsuarioCreacion", "TipoVenta");
            $this->insertDetalleFactura($arrayInsertDetalleFactura, $tipoZPBE);
        }
        DB::commit();
        /*} catch (\Exception $e) {
            DB::rollBack();
            dd($catProducto->get($codProducto, $value)
                ->where("TipoPresentacion", "Venta Inst")->last());
            $mensajeReturn = ["Tipo" => "error", "Descripcion" => "Algo salio mal al momneto de guardar la información, por favor cominiquese con el administrador del sistema"];
            session()->push('msg', $mensajeReturn);
            return redirect("/pointex/bi/ventas/cargadas");
        }*/

        return redirect("/pointex/bi/ventas/consolidacion?mes=$mes&anio=$anio&tipo=$tipoZPBE");
    }

    public function returnIdFactura($arrayInsert, $tipoZPBE, $Fecha)
    {
        $modelFacturaEncabezado = strpos($tipoZPBE, "ZPBE") !== false ? new PX_BI_FacturaEncabezadoB() : new PX_BI_FacturaEncabezado();
        //echo "$modelFacturaEncabezados<br>";
    ...
    public function insertDetalleFactura($arrayInsert, $tipoZPBE)
    {
        if (strpos($tipoZPBE, "ZPBE") !== false) {
            ...
            //echo "PX_BI_FacturaDetalleB::insert()<br>";
        } else {
            PX_BI_FacturaDetalle::insert($arrayInsert);
            //echo "PX_BI_FacturaDetalle::insert()<br>";
        }
    }
    ...
    public function cif(Request $request)
    {
        ...
        $stringSql = "select  * from [VW_VentaNeta2] where Fecha between '$anio-$mes-01' and CONVERT(date , DATEADD(month, ((YEAR('$anio-$mes-01') - 1900) * 12) + MONTH('$anio-$mes-01'), -1))";
        ...
        $arrayPaisesNombre = PX_SIS_Pais::select("Id", "Codigo")
            ->whereIn("Id", $data->pluck("IdPais")
                ...
                ->toArray())
            ->get()->pluck("Codigo", "Id");
        return view("Sistema.Pointex.Modulo.BI.Ventas.Sell_In.index_Sel_In", get_defined_vars());
    }
