...
@section('js')
    ...
    <script>
        ...
        function enviaFecha(id,val,tipo){
            location.href = '/pointex/gestion/tercerizados/edit_fecha/' + id + '/'+val+ '/'+tipo;
        }
        ...
        $(".eventoClickElimina").on("click", function () {
            ...
            swal({
                ...
                if (willDelete) {
                    window.location.href = '/pointex/gestion/tercerizados/destroy/' + getNo
                }
            });
        });
        ...
        $(".cambiar").on("click", function () {
            var getNo = this.id;
            swal({
                html:'<input type="date" id="swal-FehaFinalizacion" class="swal2-input form-control">',
                title: '<i class="fas fa-retweet fa-7x danger"></i><br>¿Asignar fecha de finalización ?',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                var FehaFinalizacion = document.getElementById('swal-FehaFinalizacion').value;
                if (willDelete) {
                    window.location.href = '/pointex/gestion/tercerizados/cambia_fecha/' + getNo + '/' + FehaFinalizacion;
                }
            });
        });
        ...
        $(".cambiarActivar").on("click", function () {
            var getNo = this.id;
            swal({
                title: '<i class="fas fa-retweet fa-7x danger"></i><br>¿Cambiar el producto a tercerizado ?',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Si',
                ...
                if (willDelete) {
                    window.location.href = '/pointex/gestion/tercerizados?' + getNo
                }
            });
        });
        ...
        function validar() {
            if (!document.querySelector('input[name="txtNotificar"]:checked')) {
                swal("advertencia!", "Seleccione si el correo es sobre la creacion de productos tercerizados *si/no!", "warning")
                hasError = true;
            } else {
        ...
        function notificar(opcion) {
            ...
            if (opcion) {
                ...
                $("#txtTitulo").val('Productos Tercerizados');
                $("#txtDescripcion").val('Buen día,\n' +
                    '\n' +
                    'Se les informa que los siguientes productos ahora serán tercerizados.\n' +
                    '\n' +
                    'Saludos.');
            } else {
                $("#txtCorreoTercerizado").val(0);
                $("#txtTitulo").val('Productos no Tercerizados');
                $("#txtDescripcion").val('Buen día,\n' +
                    '\n' +
                    'Se les informa que los siguientes productos dejaran de ser tercerizados.\n' +
                    '\n' +
                    'Saludos.');
            }
        }
        ...
        $("#cmbPais").on("change", function () {
            var fillSecondary = function () {
                ...
                $.get("/pointex/gestion/tercerizados/get_cliente/" + $('#cmbPais').val(), null, function (data) {
        ...
        $("#cmbDistribuidor").on("change", function () {
            var fillSecondary = function () {
                $('#cmbProducto').empty();
                $.get("/pointex/gestion/tercerizados/get_producto/" + $('#cmbDistribuidor').val(), null, function (data) {
