...
{{--        {!! HTML::script('/Vendor/Plantillas/Apex/app-assets/vendors/js/datatable/dataTableLanguaje.js') !!}--}}
    <script>
        ...
        function enviaFecha(id,val,tipo){
            location.href = '/pointex/gestion/market_access/edit_fecha/' + id + '/'+val+ '/'+tipo;
        }
        ...
        $(".eventoClickElimina").on("click", function () {
            ...
            swal({
                ...
                if (willDelete) {
                    window.location.href = '/pointex/gestion/market_access/destroy/' + getNo
                }
            });
        });
        ...
        $(".cambiar").on("click", function () {
            var getNo = this.id;
            swal({
                title: '<i class="fas fa-retweet fa-7x danger"></i><br>¿Cambiar el producto a no market access ?',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (willDelete) {
                    window.location.href = '/pointex/gestion/market_access?' + getNo
                }
            });
        });
        ...
        $(".cambiarActivar").on("click", function () {
            var getNo = this.id;
            swal({
                title: '<i class="fas fa-retweet fa-7x danger"></i><br>¿Cambiar el producto a market access ?',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Si',
        ...
        function validar() {
            if (!document.querySelector('input[name="txtNotificar"]:checked')) {
                swal("advertencia!", "Seleccione si el correo es sobre la creacion de productos market access *si/no!", "warning")
                hasError = true;
            } else {
        ...
        function notificar(opcion) {
            ...
            if (opcion) {
                $("#txtCorreoMarketAccess").val(1);
                $("#txtTitulo").val('Productos Market Access');
                $("#txtDescripcion").val('Buenos días,\n' +
                    '\n' +
                    'Se les informa que los siguientes productos ahora serán market access.\n' +
                    '\n' +
                    'Saludos.');
            } else {
                $("#txtCorreoMarketAccess").val(0);
                $("#txtTitulo").val('Productos no market access');
                $("#txtDescripcion").val('Buenos días,\n' +
                    '\n' +
                    'Se les informa que los siguientes productos dejaran de ser market access.\n' +
                    '\n' +
                    'Saludos.');
            }
        }
        ...
        $("#cmbPais").on("change", function () {
            var fillSecondary = function () {
                ...
                $.get("/pointex/gestion/market_access/get_cliente/" + $('#cmbPais').val(), null, function (data) {
        ...
        $("#cmbDistribuidor").on("change", function () {
            var fillSecondary = function () {
                $('#cmbProducto').empty();
                $.get("/pointex/gestion/market_access/get_producto/" + $('#cmbDistribuidor').val(), null, function (data) {
